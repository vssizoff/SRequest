const {SRequest, SRequestSync, SPost, SGet, SPut, SHead, SDelete, SOptions, SConnect, SPatch, SPostSync,
    SGetSync, SPutSync, SHeadSync, SDeleteSync, SOptionsSync, SConnectSync, SPatchSync, SWebSocket,
    SGraphqlSubscription, SGraphql, SGraphqlQM, SGraphqlS, SGraphqlSync} = (() => {
    ...
    async function SGraphql(url, query, {params, query: queryParams, ...options} = defaultGqlOptions) {
    ...
    async function SGraphqlQM(url, query, options) {
        if (url.startsWith("ws")) return 0;
        return SGraphql(url, query, options);
    }

    async function SGraphqlS(url, query, options) {
        if (url.startsWith("http")) return 0;
        return SGraphql(url, query, options);
    }

    function SGraphqlSync(url, query, {params, query: queryParams, ...options} = defaultGqlOptions) {
        url = SRequestParsers.params(url, params) + (queryParams !== undefined ? SRequestParsers.query(queryParams) : "");
        return SPostSync(url, {...options, query});
    }

    return {SRequest, SRequestSync, SPost, SGet, SPut, SHead, SDelete, SOptions, SConnect, SPatch, SPostSync,
        SGetSync, SPutSync, SHeadSync, SDeleteSync, SOptionsSync, SConnectSync, SPatchSync, SWebSocket,
        SGraphqlSubscription, SGraphql, SGraphqlQM, SGraphqlS, SGraphqlSync};
})();